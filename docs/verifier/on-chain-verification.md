...
![](/img/on-chain-verification-flow.png)
...
> To set up a different query check out the [ZK Query Language section](zk-query-language.md)
...
The prerequisite is that users have the [Polygon ID Wallet app](/guides/wallet/overview.md) installed and self-issued a Credential of type `KYC Age Credential Merklized` using our [Demo Issuer](https://issuer-demo.polygonid.me/) 
...
2. `validator`: the address of the [Validators Smart Contract](https://github.com/0xPolygonID/contracts/tree/main/contracts/validators) already deployed on Mumbai. This is the contract that executes the verification on the zk proof submitted by the user. It can be of type [CredentialAtomicQuerySigValidator](/smart-contracts.md#credentialatomicquerysigvalidator) or [CredentialAtomicQueryMTPValidator](/smart-contracts.md#credentialatomicquerymtpvalidator).
...
5. `operator` is either 1,2,3,4,5,6. To understand more about the operator you can check the [zk query language](zk-query-language.md)
6. `value` represents the threshold value you are querying. In this case it is the date 01/01/2002. 

> Check out our [Smart Contract section](/smart-contracts.md) to learn more about the set of verifications executed on the zk proof.
...
### Add Proof Request Inside a QR Code 
...
Note that the request resembles in most of its parts with the one designed for [off-chain verification](https://0xpolygonid.github.io/tutorials/verifier/verification-library/request-api/). The extra part that has been added here is the `transaction_data` that includes:
...
The same proof generation request can also be delivered to users via Deep Linking. In order to do so, it is necessary to [encode](https://www.base64encode.org/) the `json` file to Base64 Format. The related deep link would be `iden3comm://?i_m={{base64EncodedJsonHere}}`.

For example, in this specific case the deep link would be:
...
This video shows how a user can use their Polygon ID wallet app to claim a ERC-20 token airdrop. To join the airdrop users are required to have a Credential of type `KYCAgeCredential` attesting that they have been born before 01/01/2002.
...
![](/img/qr-code-on-chain-verification.png)

### How Proof Submission is Executed
