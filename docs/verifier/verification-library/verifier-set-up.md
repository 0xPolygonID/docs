...
The prerequisite is that users have self-issued a Credential of type `KYC Age Credential Merklized` using our [Demo Issuer](https://issuer-demo.optimismid.me/) on their Web Wallet or Privado Wallet App. Further credentials can be issued using the [Issuer Node](/docs/issuer/issuer-overview.md).
...
> If created using optimism ID Platform, the schema URL can be fetched from there and pasted inside your Query.
...
Audience := "did:optimismid:optimism:amoy:2qQ68JkRcf3xrHPQPWZei3YeVzHPP58wYNxx2mEouR"
...
async function GetAuthRequest(req, res) {
  ...
  const audience = "did:optimismid:optimism:amoy:2qQ68JkRcf3xrHPQPWZei3YeVzHPP58wYNxx2mEouR";
...
const resolvers = {
	['optimism:main']: mainStateResolver,
};
```
...
async function callback(req, res) {
  ...
  const resolvers = {
      ["optimism:amoy"]: new resolver.EthStateResolver(
      "<AMOY_RPC_URL>",
